/**
 * Use case: remove ONE user-added exercise occurrence from an in-progress
 * workout session (M11).
 *
 * Same guard chain as skip/unskip and move (validate → load → ownership →
 * enrollment → stale rendered intent → domain → optimistic-concurrency save).
 * The ORDER of the last two steps is the point: the stale-intent guard runs
 * BEFORE the domain removal interprets `exerciseOrder`, so a tab rendered
 * against an older session can never remove whichever occurrence now occupies
 * that old order.
 *
 * Every removal invariant (completion immutability, occurrence existence,
 * template-authored protection, logged-set protection) is delegated to the
...
export type RemoveSessionExerciseError =
  ...
  // unknown occurrence, template-authored protection, logged-set protection)
  // surface with their own codes — the removal invariants stay owned by the
  // domain.
...
export interface RemoveSessionExerciseInput {
  ...
  /**
   * Identifies the occurrence within its session — the business locator.
   * `occurrenceKey` is deliberately not an input: it is a presentation token,
   * never an address.
   */
   ...
   * `exerciseOrder` is interpreted, so a stale tab cannot remove the
...
export class RemoveSessionExerciseUseCase {
  ...
  async execute(
  ...
  ): Promise<Result<WorkoutSessionDto, RemoveSessionExerciseError>> {
    ...
    const result = removeSessionExercise(session, { exerciseOrder: input.exerciseOrder });
    if (!result.ok) {
      return result;
    }
