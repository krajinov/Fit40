/**
 * Use case: revert one skipped exercise occurrence back to not-skipped in an
 * in-progress workout session (M10).
 *
 * Same guard chain as skip (validate → load → ownership → enrollment →
 * domain → optimistic-concurrency save). Every unskip invariant (immutability
 * at completion, occurrence existence, not-currently-skipped) is delegated
 * to the domain service and never duplicated here.
 *
...
export type UnskipSessionExerciseError =
  ...
  // codes — the unskip invariants stay owned by the domain.
...
export interface UnskipSessionExerciseInput {
   ...
   * reorder cannot unskip the occurrence that now occupies its old order.
   */
  readonly expectedSessionVersion: number;
}

export class UnskipSessionExerciseUseCase {
  constructor(private readonly sessionRepository: WorkoutSessionRepository) {}

  async execute(
    input: UnskipSessionExerciseInput,
  ): Promise<Result<WorkoutSessionDto, UnskipSessionExerciseError>> {
    ...
    const result = unskipSessionExercise(session, { exerciseOrder: input.exerciseOrder });
    if (!result.ok) {
      return result;
    }
