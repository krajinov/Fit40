 ...
 * (desktop) / above them (mobile). RPE is an optional fourth input (1–10):
 * leaving it empty submits no `rpe` field at all, which the schema
 * normalizes to null.
 *
...
export function SetLoggerForm({
  ...
  async function submitAction(
    ...
    // RPE is optional: submit it only when the athlete entered one; an empty
    // field is omitted and normalizes to null in the schema.
    if (rpe !== '') {
      formData.set('rpe', rpe);
    }
        ...
        <div className="grid grid-cols-3 gap-2 md:flex md:items-end md:gap-3">
          ...
          <div>
            <label
              ...
              className="mb-1.5 block text-[11px] font-medium text-ink-2 md:text-[13px]"
            >
              <span className="md:hidden">RPE</span>
              <span className="hidden md:inline">RPE (optional)</span>
            </label>
            <Input
              id={rpeId}
              name="rpe"
          ...
          <button
            ...
            className={cn(
              buttonVariants(),
              'col-span-3 h-12 rounded-[10px] md:col-span-1 md:h-auto md:w-auto',
            )}
          >
