...
describe('DeleteSessionSetUseCase', () => {
  it('deletes a set and returns DTO', async () => {
    ...
    const r = await uc.execute({ sessionId: sessionId as string, userId: OWNER_ID, exerciseOrder: 1, setNumber: 1 , expectedSessionVersion: 1 });
  ...
  it('returns SESSION_NOT_FOUND', async () => {
    ...
    const r = await uc.execute({ sessionId: 'unknown', userId: OWNER_ID, exerciseOrder: 1, setNumber: 1 , expectedSessionVersion: 0 });
  ...
  it('returns FORBIDDEN when the session belongs to another user', async () => {
    ...
    const uc = new DeleteSessionSetUseCase(repo);
    const r = await uc.execute({ sessionId: sessionId as string, userId: 'user-2', exerciseOrder: 1, setNumber: 1 , expectedSessionVersion: 0 });
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.error.code).toBe('FORBIDDEN');
  });

  it('rejects deleting a set from a detached session (enrollment nulled by leaving)', async () => {
    const { repo, sessionId } = await sessionWithSet(OWNER_ID, null);
    const uc = new DeleteSessionSetUseCase(repo);

    const r = await uc.execute({ sessionId: sessionId as string, userId: OWNER_ID, exerciseOrder: 1, setNumber: 1 , expectedSessionVersion: 0 });
