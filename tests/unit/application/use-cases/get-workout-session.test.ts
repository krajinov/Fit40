...
function makeUseCase() {
  ...
  const programRepo: ProgramRepository = { list: vi.fn(), findBySlug: vi.fn().mockResolvedValue(program) };
...
describe('GetWorkoutSessionUseCase', () => {
  ...
  it('returns PROGRAM_NOT_FOUND', async () => {
    const programRepo: ProgramRepository = { list: vi.fn(), findBySlug: vi.fn().mockResolvedValue(null) };
