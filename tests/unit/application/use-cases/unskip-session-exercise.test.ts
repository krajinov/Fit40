...
describe('UnskipSessionExerciseUseCase', () => {
  it('unskips a skipped occurrence, persists it, and returns the updated DTO', async () => {
    ...
    expect(r.data.skippedExerciseCount).toBe(0);
    expect(r.data.prescribedSets).toBe(3);
    // The unskip persisted.
  ...
  it('returns FORBIDDEN when the session belongs to another user', async () => {
    ...
    expect(untouched?.exerciseLogs[0]?.isSkipped).toBe(true);
  });

  it('rejects unskipping a detached session (enrollment nulled by leaving)', async () => {
  ...
  it('rejects unskipping a completed session', async () => {
  ...
  it('returns ADJUSTMENT_NO_CHANGE for an occurrence that is not skipped', async () => {
    const { repo, sessionId } = await seedSession();
    const uc = new UnskipSessionExerciseUseCase(repo);
  ...
  it('never performs an exercise catalog lookup', () => {
    // Unskipping never changes the performed exercise, so the use case takes
    // ONLY the session repository (constructor arity 1, unlike substitution's
    // 2) — there is no catalog dependency to consult.
    expect(UnskipSessionExerciseUseCase.length).toBe(1);
    new UnskipSessionExerciseUseCase(new InMemoryWorkoutSessionRepository());
  });
});
